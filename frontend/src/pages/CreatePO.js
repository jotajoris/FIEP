...
const CreatePO = () => {
  ...
  // ============ HANDLERS NCM ============
  const handleNcmDragEnter = useCallback((e) => {
    e.preventDefault();
    e.stopPropagation();
    setIsDraggingNcm(true);
  }, []);

  const handleNcmDragLeave = useCallback((e) => {
    e.preventDefault();
    e.stopPropagation();
    setIsDraggingNcm(false);
  }, []);

  const handleNcmDragOver = useCallback((e) => {
    e.preventDefault();
    e.stopPropagation();
  }, []);

  const handleNcmDrop = useCallback((e) => {
    e.preventDefault();
    e.stopPropagation();
    setIsDraggingNcm(false);
    ...
    if (files.length > 0) {
      setNcmFiles(prev => [...prev, ...files]);
    }
  }, []);

  const handleNcmFileSelect = (e) => {
    const files = Array.from(e.target.files).filter(
      f => f.type === 'application/pdf' || f.name.toLowerCase().endsWith('.pdf')
    );
    if (files.length > 0) {
  ...
  const removeNcmFile = (index) => {
    setNcmFiles(prev => prev.filter((_, i) => i !== index));
  };
  ...
  const handleUpdateNcm = async () => {
    ...
    try {
      ...
      const response = await apiPost(`${API}/admin/atualizar-ncm-em-massa`, formData);

      setNcmResults(response.data);
      setNcmFiles([]);
    } catch (error) {
      console.error('Erro ao atualizar NCMs:', error);
      const errorData = error.response?.data;
      let errorMessage = 'Erro ao atualizar NCMs';
