...
async def download_backup(token: str = None, current_user: dict = Depends(require_admin)):
    """Download backup simples e rápido"""
    from datetime import datetime
    from fastapi.responses import Response
    import json
    
    try:
        logger.info("Gerando backup...")
        
        # Buscar dados sem campos pesados
        projection = {"_id": 0, "pdf_original": 0, "nf_pdf": 0, "imagem_base64": 0}
        ...
        backup = {
            "backup_info": {
                ...
                "versao": "4.3",
                "estatisticas": {
                    "ocs": len(pos), "itens": total_items, "rastreios": itens_rastreio,
                    "usuarios": len(users), "referencias": len(reference_items),
                    "limites": len(limites), "estoque": len(estoque)
                }
            },
            "purchase_orders": pos,
            "users": users,
        ...
        logger.info(f"Backup: {len(json_str)} bytes")
...
async def atualizar_oc_com_pdf(
    ...
    for idx, existing_item in enumerate(existing_items):
        ...
        if pdf_matches:
            ...
            # PREÇO DE VENDA - SEMPRE atualiza com o valor do PDF se diferente
            ...
            if preco_pdf_novo is not None and preco_pdf_novo != preco_venda_atual:
                ...
                alteracoes.append(f"preco_venda: {preco_venda_atual} → {preco_pdf_novo}")
...
# ============== ESTOQUE (Endpoints adicionais que não estão em estoque_routes.py) ==============
# Nota: O endpoint GET /estoque principal foi movido para routes/estoque_routes.py
...
# Nota: Endpoints /estoque/mapa e /estoque/verificar/{codigo_item} movidos para routes/estoque_routes.py
...
default_origins = [
    ...
    "https://orderflow-212.preview.emergentagent.com",
