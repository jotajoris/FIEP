...
async def get_purchase_orders_simple(
    ...
    for po in pos:
        ...
        result.append({
            ...
            # Incluir itens simplificados para filtro local (com quantidade e status para resumo)
            "items": [{"codigo_item": i.get('codigo_item'), "descricao": i.get('descricao'), "responsavel": i.get('responsavel'), "quantidade": i.get('quantidade', 1), "status": i.get('status')} for i in items],
            # Resumo por status
            "status_count": {
